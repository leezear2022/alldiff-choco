...
//        varsTmp = new SparseSet(arity);
        ...
        //算match时会用到
        activeVars = env.makeLong(lastMask);
    }
...
//            updatedVars.fill();
//            updatedVals.fill();
//            System.out.println(Long.toBinaryString(activeVars.get()));
...
//        dealChanges();
    ...
    private void printRD() {
        // 填充B和D
        for (int i = 0; i < arity; ++i) {
            IntVar v = vars[i];
            System.out.println(v);
            System.out.println(Long.toBinaryString(RD[i].get()));
        }
    }
...
//            System.out.printf("MakeAugmentingPath node:%d,%s\n", node, Long.toBinaryString(RD[node].get()));

            for (int valIdx = nextSetBit(RD[node], 0); valIdx >= 0;
                 valIdx = nextSetBit(RD[node], valIdx + 1)) {
//                if (numCall == 1 && valIdx == 63) {
...
//            System.out.println(graphLinkedFrontier[varIdx]);
    ...
    private boolean filterDomainsPartition(int sccStartIndex) throws ContradictionException {
        boolean filter = false;
//        System.out.println(varIdx2 + "," + partition);
//        if (numCall == 328) {
//        System.out.println(partition);
//        }
        partition.setIteratorIndexBySCCStartIndex(sccStartIndex);
