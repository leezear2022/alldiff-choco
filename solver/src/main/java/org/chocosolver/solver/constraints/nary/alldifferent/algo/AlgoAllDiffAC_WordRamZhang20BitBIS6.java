 ...
 */
public class AlgoAllDiffAC_WordRamZhang20BitBIS6 {
...
//    private SparseSet notGamma, notA;
    ...
    // CONSTRUCTORS
    //***********************************************************************************
    public AlgoAllDiffAC_WordRamZhang20BitBIS6(IntVar[] variables, ICause cause, Model model) {
        id = num++;
...
//            System.out.println(partition);
            partition.prepartitionAndGetMask(sccStartIndex, restriction, gammaMask, freeNodes);
            // 初始化DE
            hasSCCSplit = false;
            DE.clear();
            for (int i = restriction.nextSetBit(0); i != -1; i = restriction.nextSetBit(i + 1)) {
                var iter = deletedValues[i].iterator();
                while (iter.hasNext()) {
                    int valIdx = iter.next();
//                for (int j = 0, end = deletedValues[i].size(); j < end; j++) {
//                    int valIdx = deletedValues[i].get(j);
...
//        System.out.println("partition: " + partition);
        for (int varIdx = restri.nextSetBit(0); varIdx != -1; varIdx = restri.nextSetBit(varIdx + 1)) {
