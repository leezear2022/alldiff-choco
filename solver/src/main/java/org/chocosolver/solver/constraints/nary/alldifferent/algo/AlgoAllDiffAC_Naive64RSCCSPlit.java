 ...
 */
public class AlgoAllDiffAC_Naive64RSCCSPlit extends AlgoAllDiffAC_Naive {
    ...
    // bitRDom bitRBel;
    private long[] D, B;
    private IStateLong[] RD, RB;
    boolean[] DomIsRegular;

    // 内置的bit方法专用的常量
    protected long lastMask;

    protected IStateLongPartition partition;
...
//    protected static final int MOD_MASK_INT = 0x3f;
    ...
    // CONSTRUCTORS
    //***********************************************************************************
    public AlgoAllDiffAC_Naive64RSCCSPlit(IntVar[] variables, ICause cause, IEnvironment env) {
...
//        System.out.println(Arrays.toString(idx2Val));
        ...
        D = new long[numValue];
        B = new long[arity];
        RD = new IStateLong[numValue];
        RB = new IStateLong[arity];
        ...
        for (int i = 0; i < arity; ++i) {
            IntVar v = vars[0];
            var iter = v.getValueIterator(true);
            while (iter.hasNext()) {
                int a = iter.next();
                int idx = val2Idx.get(a);
            }
        }
        ...
        //for reversible bitDom and bitBelong
    }
...
//        System.out.println(freeNode);
    ...
    private void distinguish() {
        ...
        for (int varIdx = nextSetBit(gammaFrontier, 0);
            ...
            gammaFrontier |= D[valIdx] & ~gammaMask;
            // 除去第i个变量
    ...
    private void initiateMatrix() {
        ...
        while (notGamma.hasNextValid()) {
            ...
            if (!vars[varIdx].isInstantiated()) {
                graphLinkedMatrix[varIdx] = D[var2Val[varIdx]] & ~gammaMask;
