 ...
 */
public class AlgoAllDiffAC_WordRamZhang20BIS2 {
...
//    protected INaiveBitSet[] D;
    ...
    // CONSTRUCTORS
    //***********************************************************************************
    public AlgoAllDiffAC_WordRamZhang20BIS2(IntVar[] variables, ICause cause, Model model) {
        id = num++;
...
//                            Dbit[yIdx].clear(val2Idx.get(xVal));
    ...
    protected boolean propagate_SCC_filter() throws ContradictionException {
        ...
        while (changedSCCStartIndex.hasNextValid()) {
            ...
            partition.getPartitionBitSetMaskAndVars(sccStartIndex, restriction, varsTmp, DEVars, valsTmp, arity, numNodes);
...
//            System.out.println("resetVar: " + i);
            ...
            numDE += deletedValues[i].validSize();
//            DEVars.add(i);
//            deletedValues[i].iterateValid();
//            while (deletedValues[i].hasNextValid()) {
//                int valIdx = deletedValues[i].next();
...
//            System.out.println(curNode + ", " + (addArity + newNode) + ", " + unVisitedValues.get(newNode));
            if (!unVisitedValues.get(newNode)) {
                if (valIsInStack.get(newNode)) {
                    ...
                    if (numCall != 0 && numDE != 0 && !unconnected) DETest(valLowLink[newNode], maxDFS - 1);
                }
            } else {
        ...
        if (numCall != 0 && !unconnected && (numDE == 0)) {
...
//            System.out.println((addArity + curNode) + ", " + matchedVar + ", " + unVisitedVariables.get(matchedVar));
            if (!unVisitedVariables.get(matchedVar)) {
                if (varIsInStack.get(matchedVar)) {
                    ...
                    if (numCall != 0 && numDE != 0 && !unconnected) DETest(varLowLink[matchedVar], maxDFS - 1);
                }
            } else {
...
//            System.out.println((addArity + curNode) + ", " + arity + ", " + sinkIsUnvisited);
            if (!sinkIsUnvisited) {
                if (sinkIsInStack) {
                    ...
                    if (numCall != 0 && numDE != 0 && !unconnected) DETest(sinkLowLink, maxDFS - 1);
                }
            } else {
                strongConnectSink();
...
//        if (!unconnected) {
        cycles.add(lowLinkNewNode, dfs);
        DEVars.iterateValid();
        while (DEVars.hasNextValid()) {
            int i = DEVars.next();
//            deletedValues[i].iterateValid();
//            while (deletedValues[i].last()) {
//                int j = deletedValues[i].next();
            int j = deletedValues[i].last();
                while (!valsTmp.contains(j)||inCycles(i, j)) {
                    deletedValues[i].remove();
                    numDE--;
                }
//            }
...
//        System.out.println("inc:" + nodePair.a + "," + nodePair.b + "=" + varDFSNum[nodePair.a] + "," + valDFSNum[nodePair.b]);
    ...
    private boolean inCycles(int c, int d) {
//        IntTuple2 tt;
//        getIntTuple2(nodePair, f);
//        System.out.println("inc:" + nodePair.a + "," + nodePair.b + "=" + varDFSNum[nodePair.a] + "," + valDFSNum[nodePair.b]);
        // 小的存入a大的存入b
        if (varDFSNum[c] == Integer.MAX_VALUE || valDFSNum[d] == Integer.MAX_VALUE) {
            return false;
        }

        int a, b;
        if (varDFSNum[c] <= valDFSNum[d]) {
            a = varDFSNum[c];
            b = valDFSNum[d];
        } else {
            a = valDFSNum[c];
