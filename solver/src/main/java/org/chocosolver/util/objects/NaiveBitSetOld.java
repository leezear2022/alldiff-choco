...
public class NaiveBitSetOld {
    ...
    public NaiveBitSetOld(int nbits) {
        this.bitSize = nbits;
    ...
    public void set(int startIndex, NaiveBitSetOld b) {
//        for(int i = wordIndex(startIndex);)
    }

    public void naiveSet(int s, NaiveBitSetOld nbs) {
        int a = wordIndex(s);
        int b = wordOffset(s);
        // 先确定怎么偏移
        if (nbs.longSize == 1) {
...
//        }
    ...
    public void naiveAnd(int s, NaiveBitSetOld nbs) {
        int a = wordIndex(s);
        int b = wordOffset(s);
        // 先确定怎么偏移
        if (nbs.longSize == 1) {
...
//            int offset1 = nbs.bitSize + s - BITS_PER_WORD;
    ...
    public void setThenAnd(NaiveSparseBitSet a, NaiveBitSetOld b) {
        for (int i = 0, len = a.longSize; i < len; ++i) {
            int offset = a.index[i];
            this.words[offset] |= a.words[i] & b.words[offset];
        }
    ...
    public void setThenAnd(LargeBitSetOld a, NaiveBitSetOld b) {
    ...
    // 如果有，返回第一个相交的值
    // 如果没有，返回-1
    public int isIntersect(NaiveBitSetOld s) {
        long word;
        for (int i = 0; i < longSize; ++i) {
    ...
    // 如果有，返回第一个相交的值
    // 如果没有，返回-1
    public boolean isIntersect(LargeBitSetOld s) {
        long word;
    ...
    public final static void and(NaiveBitSetOld res, NaiveBitSetOld a, NaiveBitSetOld b) {
        for (int i = 0, len = res.longSize; i < len; ++i) {
            res.words[i] = a.words[i] & b.words[i];
        }
    }

    public final static void and(NaiveBitSetOld res, NaiveBitSetOld a, NaiveBitSetOld b, NaiveBitSetOld c, NaiveBitSetOld d) {
        for (int i = 0, len = res.longSize; i < len; ++i) {
            res.words[i] = a.words[i] & b.words[i] & c.words[i] & d.words[i];
        }
    }

    public final static boolean EmptyAnd(NaiveBitSetOld a, NaiveBitSetOld b) {
...
//        boolean res = false;
    ...
    public final static boolean EmptyAnd(NaiveBitSetOld a, NaiveBitSetOld b, NaiveBitSetOld c, NaiveBitSetOld d) {
        for (int i = 0; i < a.longSize; ++i) {
    ...
    public final static int NextSetBitAfterMinus(NaiveBitSetOld a, NaiveBitSetOld b, int fromIndex) {
        if (fromIndex < 0) {
            throw new IndexOutOfBoundsException("fromIndex < 0: " + fromIndex);
        } else {
    ...
    // 从a中除去b，再加入现在的值
    public void orAfterMinus(NaiveBitSetOld a, NaiveBitSetOld b) {
        for (int i = 0; i < longSize; ++i) {
            this.words[i] |= a.words[i] & ~b.words[i];
        }
    }
    ...
    // 从a中除去b
    public void setAfterMinus(NaiveBitSetOld a, NaiveBitSetOld b) {
        for (int i = 0; i < longSize; ++i) {
            this.words[i] = a.words[i] & ~b.words[i];
        }
    }
    ...
    // a和b共同部分
    public void setAfterAnd(NaiveBitSetOld a, NaiveBitSetOld b) {
        for (int i = 0; i < longSize; ++i) {
            this.words[i] = a.words[i] & b.words[i];
        }
    }
    ...
    // 与a取反
    public void andAfterNot(NaiveBitSetOld a) {
        for (int i = 0; i < longSize; ++i) {
